...
For GlamGo specifically, Spline will be used for:

- **Landing page hero** — 3D background scene (desktop only, static fallback on mobile)
- **Gallery enhancements** — if feasible within the 4-day sprint

Since GlamGo uses **Next.js 14**, always use `@splinetool/react-spline/next` with `dynamic(() => import(...), { ssr: false })` to prevent hydration errors.
